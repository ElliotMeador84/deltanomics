...
# retrieve all census tract polygons per county
top_5_20_sf_ls <- 
...
`HTML` is essential a coding approach to formatting web applications. 
Officially `HTML` is: 

>Hypertext Markup Language (HTML) is the standard markup language for documents designed to be displayed in a web browser. It can be assisted by technologies such as Cascading Style Sheets (CSS) and scripting languages such as JavaScript.

First, we'll use `HTML` to create some tooltips that provide users information when they hover over polygons in the map. 
...
tooltip <- top_5_20_sf %>% 
  ...
  mutate(county = str_to_title(county), 
         ...
         rate = percent(rate)) %>% 
  transmute(tip = glue('County: {county} <br> State: {state} <br> Distance to supermarket <br> {dist} <br> SNAP percent: {rate} <br> Tract: {census_tract}'))


l_ii <- leaflet(top_5_20_sf) %>% 
  addProviderTiles("OpenStreetMap") %>%
  addPolygons(color = "black", 
              popup = tooltip$tip) 

frameWidget(l_ii, width = "100%")
